# china-tax 项目开发规范 (AI 指引手册 - v4.0.1)
...
## 3.5 PDF ➔ MD 结构化自适应提取架构与数据清洗设计规范
为了从非结构化或半结构化的金融对账单（PDF）中提取出无损、精准的财务明细，必须严格遵守以下自适应清洗漏斗架构规范：
1. **智能自适应防御（Adaptive Defense）**：
   - 提取文本前，必须前置读取样本页的文本数据流进行**可读度与乱码校验**。
   - **乱码特征指标**：计算文本中私有字符集（PUA，`\uE000-\uF8FF`）占比（不得高于10%），且结合金融高频词汇（如 date/amount/日期/金额等）进行可读性分析。
   - **动态切换 OCR**：如果检测到样本属于无字扫描件或字体编码混淆乱码，必须自动降级激活 ONNX Runtime GPU/CUDA 推理引擎进行深度扫描；若验证为可用文本 PDF，则禁用 OCR，采用纯数字提取，以获得 10 倍以上的解析速度。
2. **券商样板文本过滤（Boilerplate Filtering）**：
   - 如果表格的列头或首行被误解析为券商名称、地址、电话、邮箱、网址等样板文本，必须重置该表的列标题为 generic 数字序号（如 `0, 1, 2...`）。
   - 被重置的样板文本行留在 DataFrame 内部，会在后续的无数字和无日期检测阶段被自动过滤。
3. **数据对账与数字清洗**：
   - **负数还原**：必须提取财务常用的各种负数表达（括号负数 `(123.45)`、带空格负数 `- 123.45`、尾部负数 `123.45-`），剥离非数字符号后强制重新合并负号输出。
   - **列名唯一化**：清洗前必须进行列名去重（如重名自动重命名为 `col_1`, `col_2`），防范 Pandas 多重索引引发的类型退化崩溃。
   - **折行缝合与向下填充**：
     - 若明细行中某单元格（如长股票名称）被换行截断，必须向下定位并利用 Python 字典将截断内容向上缝合。
     - 若明细行存在省略的标的代码，必须通过 `ffill()` 在本表边界内安全向下填充。
4. **追溯性备注**：
   - 必须为每一行提取出的明细数据记录附加 `数据源备注` 属性，标记物理页码和坐标行号（如：`该行提取自原 PDF 的第 {page} 页第 {line} 行`），以确保对账链路的可追溯性。

## 4. 开发进度 (Progress Tracker)
- [x] **app.py (主入口)**: 已完成 NiceGUI 主控制面板，完全拆分轻量化（< 200行）。
- [x] **utils/csv_excel.py (第1模块 - 互转工具)**: 已开发完成。支持 `dtype=str` 长数字安全原则，并成功修复了多表变长列 CSV 转换为 Excel 的 tokenization 崩溃错误。
...
- [x] **utils/pdf_md.py (第3模块 - PDF转Markdown)**: 重构完成。基于 IBM Docling + Pandas 黄金架构重写，支持子表提取、数字纠错、重复表头剔除、汇总行前置拦截、向上折行缝合及向下顺延填充。并解决了新版 export_to_dataframe 的弃用警告。
...
- [x] **全链路转换回环验证 (Conversion Round-trip Verification)**: 成功验证 PDF ➔ MD ➔ CSV ➔ Excel ➔ CSV 的全链条回环，实现 100% 字符级无损对准，全面保证财务申报数据的准确无误。
